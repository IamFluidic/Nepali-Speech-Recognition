...
class AudioPreprocessor:
    ...
    def frame_blocking(self, y, sr, frame_length=0.025, hop_length=0.010):
        flen = int(frame_length * sr)
        hlen = int(hop_length * sr)
        if len(y) < flen:
            y = np.pad(y, (0, flen - len(y)), mode='constant')
        frames = librosa.util.frame(y, frame_length=flen, hop_length=hlen)
        return frames
...
def preprocess_feature(file_path, max_duration_sec=120.0, apply_vad=True):
    try:
        ...
        if isinstance(file_path, str):
            audio_arr, sr = _sf.read(file_path)
        elif isinstance(file_path, np.ndarray):
            audio_arr, sr = file_path, 16000
        else:
            audio_arr, sr = _sf.read(file_path)

        if sr != 16000:
            audio_arr = _librosa.resample(audio_arr.astype(np.float32), orig_sr=sr, target_sr=16000)
            sr = 16000
        ...
        mfcc = _librosa.feature.mfcc(y=audio_arr, sr=sr, n_mfcc=13, n_fft=512, hop_length=160)
        delta = _librosa.feature.delta(mfcc)
        delta2 = _librosa.feature.delta(mfcc, order=2)
        ...
        mfccs, delta_mfccs, delta_delta_mfccs = preprocessed_mfcc_features.preprocess_audio(file_path)
        min_frames = min(mfccs.shape[1], delta_mfccs.shape[1], delta_delta_mfccs.shape[1])
        return np.concatenate((mfccs[:, :min_frames], delta_mfccs[:, :min_frames], delta_delta_mfccs[:, :min_frames]), axis=0)
